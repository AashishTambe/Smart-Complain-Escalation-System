...
public class UserDAO {
    ...
    public User findByIdentifierAndPassword(String identifier, String password) throws SQLException {
        String sql = "SELECT * FROM users WHERE (email = ? OR phone = ?) AND password = ? AND active = 1";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, identifier);
            ps.setString(2, identifier);
            ps.setString(3, password); // for demo only; use hashing in production
